...
tissue_types = ['adipose', 'background', 'debris', 'lymphocytes', 'mucus', 'smooth_muscle', 'normal_colon_mucosa', 'cancer_associated_stroma', 'colorectal_adenocarcinoma_epithelium']
...
tissue_types_dict = {'adipose':'ADI', 'background':'BACK', 'debris':'DEB', 'lymphocytes':'LYM', 'mucus':'MUC', 'smooth_muscle':'MUS', 'normal_colon_mucosa':'NORM', 'cancer_associated_stroma':'STR', 'colorectal_adenocarcinoma_epithelium':'TUM'}

# move data to train directory
for tissue_type in tissue_types:
    source_directory = os.getcwd() + '/NCT-CRC-HE-100K/' + tissue_types_dict[tissue_type] + '/'
    destination_directory = os.getcwd() + '/nct_crc_he_100k_train/' + tissue_type
    images = os.listdir(source_directory)
    for image in images:
        shutil.move(source_directory + image, destination_directory)

shutil.rmtree(os.getcwd() + '/NCT-CRC-HE-100K')

# move data to validation and test directories
def move_to_directory(directory, tissue_type, total, number_to_move):
    files = [ f.path for f in os.scandir(os.getcwd() + '/nct_crc_he_100k_train/' + tissue_type)]
...
for tissue_type in tissue_types:
    total = len([name for name in os.listdir(os.getcwd() + '/nct_crc_he_100k_train/' + tissue_type)])
